...
function validateCamera(camera) {
  ...
  if (!camera.enablePTZ || (camera.sourceType !== "ONVIF" && camera.sourceType !== "RTSP+ONVIF")) {
    const error = new Error("PTZ hanya tersedia untuk kamera ONVIF atau RTSP+ONVIF dengan PTZ aktif");
    error.status = 400;
    throw error;
  }
}
